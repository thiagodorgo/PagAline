...
import { format, isSameMonth, isPast, isToday, isTomorrow, parseISO } from "date-fns";
...
function BillItemWithSwipe({ bill, onPay, onSchedule, onLongPress }: { 
  bill: Bill; onPay: (id: string) => void; onSchedule: (bill: Bill) => void; onLongPress: (bill: Bill) => void;
}) {
  const controls = useAnimation();
  const timerRef = useRef<NodeJS.Timeout | null>(null);
  const [dragOffset, setDragOffset] = useState(0);

  const handleTouchStart = () => { timerRef.current = setTimeout(() => onLongPress(bill), 2000); };
  ...
  const handleDragEnd = async (_e: any, info: PanInfo) => {
    ...
    if (info.offset.x > threshold || (info.offset.x > 30 && velocity > 500)) {
      if (bill.status !== 'paid') {
        controls.start({ x: 500, transition: { duration: 0.2 } }).then(() => { onPay(bill.id); controls.set({ x: 0 }); });
      ...
      if (bill.status !== 'paid') { onSchedule(bill); }
      controls.start({ x: 0, transition: { type: "spring", stiffness: 300, damping: 25 } });
    } else { controls.start({ x: 0, transition: { type: "spring", stiffness: 300, damping: 25 } }); }
  };

  const getStatusBadge = (bill: Bill) => {
    if (bill.status === 'paid') return <Badge variant="outline" className="text-success border-success/30 bg-success/10">Pago</Badge>;
    if (bill.status === 'overdue') return <Badge variant="outline" className="text-destructive border-destructive/30 bg-destructive/10">Vencido</Badge>;
      ...
      {bill.status !== 'paid' && (
        <div className="absolute inset-0 flex justify-between items-center px-6">
          <div className={cn("flex items-center gap-2 font-medium transition-opacity", dragOffset > 40 ? "text-success opacity-100" : "text-muted-foreground opacity-50")}>
            <CheckCircle2 size={22} /><span className="text-sm">Pagar</span>
          </div>
          <div className={cn("flex items-center gap-2 font-medium transition-opacity", dragOffset < -40 ? "text-primary opacity-100" : "text-muted-foreground opacity-50")}>
            <span className="text-sm">Agendar</span><CalendarIcon size={22} />
          </div>
        </div>
      )}
      <motion.div drag={bill.status !== 'paid' ? "x" : false} dragConstraints={{ left: 0, right: 0 }} dragElastic={0.4}
        onDragStart={handleDragStart} onDrag={handleDrag} onDragEnd={handleDragEnd} animate={controls}
        ...
        className="relative z-10 touch-pan-y" whileTap={{ scale: 0.98 }} onClick={() => onLongPress(bill)}>
          ...
          <CardContent className="p-4 flex items-center justify-between">
            <div className="flex-1 min-w-0 pr-4">
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <span>{bill.category}</span><span>•</span>
                <span>Venc: {format(toDate(bill.dueDate), "dd/MM", { locale: ptBR })}</span>
                {bill.status === 'paid' && (<><span>•</span><Paperclip size={12} className="text-primary" /></>)}
              </div>
            </div>
...
export default function Bills() {
  ...
  const handlePaySingle = async (id: string) => {
    await markAsPaid(id);
    toast({ title: "Conta paga", description: "A conta foi marcada como paga e atualizada no histórico." });
  };
    ...
    <div className="flex flex-col min-h-full pb-24">
      <header className="px-6 pt-12 pb-4 bg-card border-b border-border sticky top-0 z-10">
        ...
        <div className="flex items-center justify-between bg-muted/50 rounded-full p-1 mb-4">
          <Button variant="ghost" size="icon" onClick={prevMonth} className="h-8 w-8 rounded-full"><ChevronLeft size={18} /></Button>
          <span className="font-semibold text-sm capitalize">{format(currentMonth, "MMMM yyyy", { locale: ptBR })}</span>
          <Button variant="ghost" size="icon" onClick={nextMonth} className="h-8 w-8 rounded-full"><ChevronRight size={18} /></Button>
        </div>
      ...
      <div className="px-4 py-4 flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Buscar contas..." className="w-full pl-9 pr-4 py-2 bg-muted border-none rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" />
        </div>
        <Button variant="outline" size="icon" className="shrink-0 rounded-xl border-border"><Filter size={18} /></Button>
      </div>
        ...
        {filteredBills.length === 0 ? (
          ...
          <div className="space-y-1">
            ...
            {filteredBills.map((bill) => (
              <BillItemWithSwipe key={bill.id} bill={bill} onPay={handlePaySingle} onSchedule={handleSchedule} onLongPress={handleLongPress} />
            ))}
          </div>
        )}
      </div>
        ...
        <DrawerContent>
          ...
          <div className="p-4 flex flex-col gap-2">
            {activeBill?.status !== 'paid' ? (
              ...
              <Button variant="outline" className="justify-start text-base py-6 bg-success/5 border-success/20 text-success hover:bg-success/10" onClick={() => setDrawerType('receipt')}>
                <FileText className="mr-3" size={20} /> Ver Comprovante
              </Button>
            )}
            <Button variant="outline" className="justify-start text-base py-6" onClick={() => {
              if (activeBill) {
                setEditForm({
                  description: activeBill.description, amount: activeBill.amount.toString(),
                  dueDate: new Date(activeBill.dueDate).toISOString().split('T')[0], category: activeBill.category
                });
                setDrawerType('edit');
              }
            }}>
      ...
      <Drawer open={drawerType === 'schedule'} onOpenChange={(open) => !open && setDrawerType(null)}>
        <DrawerContent>
          <DrawerHeader><DrawerTitle>Agendar Pagamento</DrawerTitle><DrawerDescription>{activeBill?.description}</DrawerDescription></DrawerHeader>
          <div className="p-4 space-y-4">
            ...
            <Button className="w-full py-6 text-lg rounded-2xl" onClick={() => {
              toast({ title: "Pagamento agendado", description: "O pagamento foi agendado para a data selecionada." }); setDrawerType(null);
      ...
      <Drawer open={drawerType === 'receipt'} onOpenChange={(open) => !open && setDrawerType(null)}>
        <DrawerContent className="h-[85vh]">
          <DrawerHeader className="border-b border-border pb-4">
            ...
            <DrawerDescription>Salvo em {activeBill?.paidDate ? format(toDate(activeBill.paidDate), "dd/MM/yyyy HH:mm") : 'Data não disponível'}</DrawerDescription>
          </DrawerHeader>
          <div className="p-6 flex flex-col items-center justify-center gap-6 h-full overflow-y-auto">
            <div className="bg-muted w-full max-w-sm rounded-xl p-6 border border-border shadow-sm flex flex-col relative overflow-hidden">
              ...
              <div className="flex flex-col items-center justify-center pb-6 border-b border-border border-dashed">
                <div className="w-12 h-12 bg-success/20 text-success rounded-full flex items-center justify-center mb-3"><CheckCircle2 size={24} /></div>
              ...
              <div className="py-6 space-y-4">
                <div className="flex justify-between items-start"><span className="text-sm text-muted-foreground">Beneficiário</span><span className="text-sm font-medium text-right max-w-[150px]">{activeBill?.description}</span></div>
                <div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Instituição</span><span className="text-sm font-medium">Banco Genérico S.A.</span></div>
                <div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Data do Pagamento</span><span className="text-sm font-medium">{activeBill?.paidDate ? format(toDate(activeBill.paidDate), "dd/MM/yyyy") : ''}</span></div>
                <div className="flex justify-between items-start"><span className="text-sm text-muted-foreground">Código</span><span className="text-xs font-mono bg-background p-1 rounded max-w-[150px] break-all text-right text-muted-foreground">00000.00000 00000.000000 00000.000000 0 00000000000000</span></div>
                <div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Autenticação</span><span className="text-xs font-mono">A1B2C3D4E5F6G7H8</span></div>
              </div>
            </div>
            <div className="flex gap-3 w-full max-w-sm mt-auto">
              <Button variant="outline" className="flex-1 gap-2"><Download size={18} /> Baixar</Button>
              <Button className="flex-1 gap-2">Compartilhar</Button>
            </div>
          </div>
        </DrawerContent>
      </Drawer>
      ...
      <Drawer open={drawerType === 'edit'} onOpenChange={(open) => !open && setDrawerType(null)}>
        <DrawerContent className="max-h-[90vh]">
          <DrawerHeader><DrawerTitle>Editar Conta</DrawerTitle><DrawerDescription>Atualize os dados da conta abaixo.</DrawerDescription></DrawerHeader>
            ...
            <Button className="w-full py-6 text-lg rounded-2xl mt-4" onClick={async () => {
              if (activeBill) {
                await updateBill(activeBill.id, {
                  description: editForm.description, amount: parseFloat(editForm.amount) || 0,
                  dueDate: editForm.dueDate ? new Date(editForm.dueDate).toISOString() : activeBill.dueDate, category: editForm.category
                });
