...
export default function Reports() {
  ...
  const categoryData = monthBills.reduce((acc: any[], bill) => {
    ...
    if (existing) { existing.value += bill.amount; } else { acc.push({ name: bill.category, value: bill.amount }); }
    return acc;
    ...
    <div className="flex flex-col min-h-full pb-24">
      <header className="px-6 pt-12 pb-6 bg-card border-b border-border sticky top-0 z-10">
        ...
        <div className="flex items-center justify-between bg-muted rounded-full p-1 border border-border/50">
          <Button variant="ghost" size="icon" onClick={prevMonth} className="h-10 w-10 rounded-full hover:bg-background"><ChevronLeft size={20} /></Button>
          <span className="font-bold text-sm capitalize">{format(currentMonth, "MMMM yyyy", { locale: ptBR })}</span>
          <Button variant="ghost" size="icon" onClick={nextMonth} className="h-10 w-10 rounded-full hover:bg-background"><ChevronRight size={20} /></Button>
        </div>
      </header>
          ...
          <CardContent className="p-5 relative z-10">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs font-medium text-primary-foreground/90">
                <span>Progresso da Meta</span><span>{formatCurrency(monthlyGoal)} limite</span>
              </div>
              <div className="h-2 w-full bg-primary-foreground/20 rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${goalPercentage > 90 ? 'bg-red-400' : goalPercentage > 75 ? 'bg-orange-300' : 'bg-white'}`} style={{ width: `${goalPercentage}%` }}></div>
              </div>
              <p className="text-[10px] text-primary-foreground/70 font-medium">
                {goalPercentage >= 100 ? "Você ultrapassou sua meta mensal!" : `Você usou ${Math.round(goalPercentage)}% da sua meta`}
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Card className="bg-card border-border shadow-sm">
            <CardContent className="p-4 flex flex-col items-center justify-center text-center">
              <div className="w-10 h-10 rounded-full bg-success/10 flex items-center justify-center text-success mb-2"><TrendingDown size={20} /></div>
          ...
          <Card className="bg-card border-border shadow-sm">
            <CardContent className="p-4 flex flex-col items-center justify-center text-center">
              <div className="w-10 h-10 rounded-full bg-destructive/10 flex items-center justify-center text-destructive mb-2"><TrendingUp size={20} /></div>
          ...
          <>
            <Card className="shadow-sm border-border">
              <CardHeader className="pb-0 pt-5">
                <CardTitle className="text-base font-bold flex items-center gap-2">
                  <div className="w-2 h-6 bg-primary rounded-full"></div>Onde você gastou
                </CardTitle>
              </CardHeader>
                  ...
                  {categoryData.map((item, index) => (
                    <div key={item.name} className="flex items-center justify-between p-3 rounded-xl bg-muted/50 hover:bg-muted transition-colors">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-muted-foreground text-xs font-medium bg-background px-2 py-1 rounded-md w-12 text-center">{Math.round((item.value / totalAmount) * 100)}%</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <Card className="shadow-sm border-border">
              <CardHeader className="pb-2 pt-5">
                <CardTitle className="text-base font-bold flex items-center gap-2">
                  <div className="w-2 h-6 bg-primary rounded-full"></div>Balanço do Mês
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-4">
